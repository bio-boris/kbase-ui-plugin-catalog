define([
    ...
    $.KBWidget({
        ...
        initMainPanel: function () {
            ...
            $mainPanel.append($('<h3>').append('Queue Summary:'));
            var $basicStatusDiv = $('<div>');
            $mainPanel.append($basicStatusDiv);

            $mainPanel.append($('<h3>').append('Jobs in Queue:'));
            //$mainPanel.append('<br>');
        ...
        renderBasicStatus: function () {
            ...
            if (self.requested_releases.length > 0) {
              ...
              self.$basicStatusDiv.append(" <table id=\"tab\"></table>");
             var table =  self.createQueueStatusTable();
               self.$basicStatusDiv.append(table);

        },


        createQueueStatusTable : function () {
           var self = this;

            var bigmemd = {held : 1, completed : 183,  running : 1, idle :0};
            var bigmemlongd = {held : 1, completed : 183,  running : 1, idle :0};
            var hpcd = {held : 1, completed : 183,  running : 1, idle :0};
            var kb_uploadd = {held : 1, completed : 183,  running : 1, idle :0};
            var njsd = {held : 1, completed : 183,  running : 1, idle :0};

            var d = {
                bigmem: bigmemd,
                bigmemlong: bigmemlongd,
                hpc: hpcd,
                kb_upload: kb_uploadd,
                njs: njsd
            }
            ...
            var adminUserStatsConfig = {
                rowsPerPage: 50,
                headers: [
                    {text: 'Client Group', id: 'clientgroup', isSortable: true},
                    {text: 'Held', id: 'held', isSortable: true},
                    {text: 'Completed', id: 'completed', isSortable: true},
                    {text: 'Running', id: 'running', isSortable: true},
                    {text: 'Idle', id: 'idle', isSortable: true}
                ]
            };

            var rows = bigmemd;
            // for (var clientgroup in d) {
            //
            //     rows+= d[clientgroup];
            //
            //
                ...
                new DynamicTable($adminUserStatsContainer,
                    {
                        ...
                        updateFunction: self.createDynamicUpdateFunction(adminUserStatsConfig, rows)
                    }
                );

                return $adminUserStatsContainer;
